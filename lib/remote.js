...
class RemoteTransform extends Transform {
    ...
    async request(method, uri, body, variables) {
        ...
        if (!['GET', 'HEAD', 'OPTIONS'].includes(method) && body) {
            if (typeof(body) === 'string') {
                body = this.interpolateUrlString(body, variables)
            }
            else {
        ...
        const { responseBody } = await scheduler.tryRequest({ method, uri, body })
    ...
    async run(nodes, options) {
        ...
        const { concurrency = Infinity } = transform

        return await super.run(nodes, options, concurrency)
    }
}
    ...
    return {
        ...
        load: function() {
            return class {
                ...
                static get options() {
                    return remoteTransform.options
                }
