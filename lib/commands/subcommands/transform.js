                ...
                handler: async(argv) => {
                    ...
                    if (transformName === 'auto') {
                        ...
                        Object.entries(transformers).forEach(([name, transform]) => {
                            const a = name && nameRegExp.test(name)
                            const b = alias && transform.alias.some((alias) => aliasRegExp.test(alias))
                            const c = title && titleRegExp.test(transform.title)
                            const d = tag && transform.tags.some((tag) => tagRegExp.test(tag))
