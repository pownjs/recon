    ...
    builder: (yargs) => {
        ...
        Object.entries({ ...transformers, auto }).forEach(([transformName, transform]) => {
            yargs.command({
                ...
                handler: async(argv) => {
                    const { noise, select, group, autoGroup, autoWeight, extract, extractPrefix, extractSuffix, nodeType, nodes, ...rest } = argv
                    ...
                    try {
                        results = await recon.transform(transformName === 'auto' ? '*' : transformName, options, { noise, group: autoGroup, weight: autoWeight, extract: { property: extract, prefix: extractPrefix, suffix: extractSuffix } })
                    }
                    catch (e) {
                        console.error(e)
                    }
