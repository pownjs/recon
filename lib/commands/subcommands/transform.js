    ...
    builder: (yargs) => {
        ...
        Object.entries({ ...transformers, auto }).forEach(([transformName, transform]) => {
            yargs.command({
                ...
                handler: async(argv) => {
                    const { noise, group, select, extract, extractPrefix, extractSuffix, nodeType, nodes, ...rest } = argv
                    ...
                    if (select) {
                        ...
                        recon.addNodes(nodes.map((node) => ({ id: makeId(), type: nodeType, label: node, props: { [nodeType]: node }, edges: [] })))
                    }

                    let results = []

                    try {
                        results = await recon.transform(transformName === 'auto' ? '*' : transformName, options, { noise, extract: { property: extract, prefix: extractPrefix, suffix: extractSuffix } })
                    }
                    catch (e) {
                        console.error(e)
                    }
