...
class Recon extends EventEmitter {
    ...
    async transform(transformation, options = {}, settings = {}) {
        const { transformConcurrency = Infinity, nodeConcurrency = Infinity, timeout = 0, group = false, weight = false, filter, extract, maxNodesWarn = this.maxNodesWarn, maxNodesCap = this.maxNodesCap, optionsInstance } = settings
