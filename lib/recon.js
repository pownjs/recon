...
class Recon extends EventEmitter {
    ...
    async transform(transformation, options = {}, settings = {}) {
        const { timeout = 0, group = false, weight = false, filter, extract, maxNodesWarn: _maxNodesWarn, maxNodesCap: _maxNodesCap } = settings
        ...
        let results = await Promise.all(transforms.map(async(transform) => {
            ...
            try {
                const tasks = [transform.run(actualNodes, options)]
