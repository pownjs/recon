...
class Recon extends EventEmitter {
    ...
    async transform(transformation, options = {}, settings = {}) {
        ...
        let results = await Promise.all(transformers.map(async(transformer) => {
            ...
            if (result.length) {
                if (group) {
                    const { title, description } = transformer.constructor

                    const label = title

                    const parentId = makeId('group', label)

                    result.unshift({ id: parentId, type: 'group', label, props: { title, description }, edges: [] })
        ...
        if (maxNodesWarn && results.length > maxNodesWarn) {
            this.emit('warn', `Attempting to add ${results.length} nodes`)
        }
