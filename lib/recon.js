...
class Recon extends EventEmitter {
    ...
    async transform(transformation, options = {}, settings = {}) {
        const { timeout = 0, group = false, weight = false, filter, extract, maxNodesWarn: _maxNodesWarn, maxNodesCap: _maxNodesCap, optionsInstance } = settings
        ...
        let results = await Promise.all(transforms.map(async(transform) => {
            ...
            const transformNames = [transform.name].concat(transformAliases)
            ...
            try {
                const tasks = [transform.run(actualNodes, actualOptions)]
