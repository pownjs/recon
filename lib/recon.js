...
class Recon extends EventEmitter {
    ...
    async * transformIt(transformation, options = {}, settings = {}) {
        ...
        let transforms = transformNames.map((transformName) => {
            ...
            const { title } = this.transforms[transformName]

            const prefix = `transform ${JSON.stringify(title)} :::`
                ...
                TransformClass = class extends this.transforms[transformName] {
                    async handle(node, options, ...args) {
                        ...
                        if (!result) {
                            ...
                            cache.set(transformName, node, options, result).catch((error) => this.emit('error', prefix, error))
                        }
