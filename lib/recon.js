...
class Recon extends EventEmitter {
    ...
    traverse(...expressions) {
        const traversors = [].concat(...expressions.map((expression) => {
            // TODO: use a better grammer / parser for this
    ...
    async transform(transformation, options = {}, settings = {}) {
        const { transformConcurrency = Infinity, runConcurrency = Infinity, timeout = 0, group = false, weight = false, filter, extract, maxNodesWarn: _maxNodesWarn, maxNodesCap: _maxNodesCap, optionsInstance } = settings
        ...
        await eachOfLimit(transforms, transformConcurrency, async(transform) => {
            ...
            try {
                const tasks = [transform.run(actualNodes, actualOptions, runConcurrency)]
