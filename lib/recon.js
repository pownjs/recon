...
const { makeId, isIterable } = require('./utils')
...
class Recon extends EventEmitter {
    ...
    async addNodes(nodes) {
        ...
        for await (let { id, type, label, props, edges = [], ...data } of nodes) {
            updateProgres()
    ...
    async * transformIt(transformation, options = {}, settings = {}) {
        ...
        for await (let results of iterateOfLimit(transforms, transformConcurrency, async(transform) => {
