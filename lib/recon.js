...
class Recon extends EventEmitter {
    ...
    async transform(transformation, options = {}, settings = {}) {
        ...
        let transformers = transformNames.map((transformName) => {
            if (!this.transforms.hasOwnProperty(transformName)) {
                throw new Error(`Unknown transformer ${transformName}`)
            }

            const transformer = new this.transforms[transformName]()
        ...
        if (transformation === '*') {
            ...
            if (filter) {
                ...
                if (name || alias || title || tag) {
                    ...
                    transformers = transformers.filter(({ constructor = {} }) => {
        ...
        transformers.sort((a, b) => a.constructor.priority - b.constructor.priority)
        ...
        let results = await Promise.all(transformers.map(async(transformer) => {
            ...
            try {
                results = await transformer.run(actualNodes, options)
            }
            catch (e) {
                results = []
            ...
            if (results.length) {
                if (group) {
                    const { group, title, description } = transformer.constructor
