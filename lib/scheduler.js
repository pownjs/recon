    ...
    async tryRequest(request) {
        ...
        for (let i = 0; i < maxRetries; i++) {
            ...
            if (res.info.error) {
                ...
                console.debug(`${res.method || 'GET'} ${res.uri} -> ${res.responseCode}`, Buffer.from(res.responseBody).toString('base64'))
            ...
            try {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    ...
                    if (process.env.POWN_DEBUG) {
                        console.error('responseBody:', res.responseBody.toString('base64'))
                    }
                }
            }
        }
