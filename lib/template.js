const { Task, createTaskSetClass, Template, createTemplateSetClass, getArray } = require('@pown/engine/lib/template')
            ...
            if (nodes.length) {
                nodes = await Promise.all(nodes.map(async(node) => {
                    ...
                    if (result) {
                        ...
                        const { id, type, label, props } = resultData
...
const AddTaskSet = createTaskSetClass(AddTask)
...
const TransformTaskSet = createTaskSetClass(TransformTask)
...
const RemoveTaskSet = createTaskSetClass(RemoveTask)
