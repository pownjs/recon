...
const githubListRepos = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        for (let page = 1; page <= pages; page++) {
            ...
            const { responseBody } = await githubApiScheduler.tryFetch(`https://api.github.com/users/${search}/repos?${query}`, this.headers)
...
const githubListGists = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        for (let page = 1; page <= pages; page++) {
            ...
            const { responseBody } = await githubApiScheduler.tryFetch(`https://api.github.com/users/${search}/gists?${query}`, this.headers)
...
const githubListMembers = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        for (let page = 1; page <= pages; page++) {
            ...
            const { responseBody } = await githubApiScheduler.tryFetch(`https://api.github.com/orgs/${search}/members?${query}`, this.headers)
