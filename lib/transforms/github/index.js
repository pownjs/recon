...
const defineTransform = (def) => {
    return class extends Transform {
        ...
        async handle({ id: source = '', label = '' }, options) {
            ...
            return def.transform.call(this, { source, label, github }, options)
        }
    }
}
    ...
    transform: async({ source, label, github }, options) => {
        ...
        for await (let { login: user, type } of github.search(label, ['users'])) {
            ...
            try {
                ...
                if (bioFilterRegex && !bioFilterRegex.test(bio || '')) {
                    continue
                }

                if (companyFilterRegex && !companyFilterRegex.test(company || '')) {
                    continue
                }
