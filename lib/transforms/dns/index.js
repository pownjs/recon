...
const dnsResolve = class extends Transformer {
    ...
    async doA({ id: target = '', label = '' }, resolve) {
        ...
        return records.map((record) => {
            return { type: IPV4_TYPE, label: record, props: { ipv4: record, type: 'A' }, edges: [target] }
        })
    }
    ...
    async doAAAA({ id: target = '', label = '' }, resolve) {
        ...
        return records.map((record) => {
            return { type: IPV6_TYPE, label: record, props: { ipv6: record, type: 'AAAA' }, edges: [target] }
        })
    }
    ...
    async doMX({ id: target = '', label = '' }, resolve) {
        ...
        return records.map((record) => {
            ...
            return { type: DOMAIN_TYPE, label: exchange, props: { domain: exchange, type: 'MX' }, edges: [target] }
        })
    }
    ...
    async doTXT({ id: target = '', label = '' }, resolve) {
        ...
        const results = records.map((records) => {
            return records.map((record) => {
                return { type: STRING_TYPE, label: record, props: { string: record, type: 'TXT' }, edges: [target] }
            })
        })
