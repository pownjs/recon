...
const dnsResolve = class extends Transform {
    ...
    async handle(item, options) {
        const { type = 'ALL', servers = '', semaphore } = options
        ...
        })(util.promisify(resolver.resolve.bind(resolver)))
    ...
    async run(items, { concurrency = DEFAULT_CONCURRENCY, ...options }) {
        const semaphore = new Semaphore(concurrency)

        return await super.run(items, { ...options, concurrency, semaphore })
    }
}
