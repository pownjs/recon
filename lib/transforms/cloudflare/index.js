    ...
    async query(name, type, target) {
        ...
        return Answer.map(({ type, data }) => {
            if (type === 1) {
                ...
                return { type: IPV4_TYPE, label: ipv4, props: { ipv4, type: 'A', domain }, edges: [target] }
            }
            else
            if (type === 28) {
                const ipv6 = data
                ...
                return { type: IPV6_TYPE, label: ipv6, props: { ipv6, type: 'AAAA', domain }, edges: [target] }
            }
            else
            if (type === 5) {
                ...
                return { type: DOMAIN_TYPE, label: domain, props: { domain, type: 'CNAME', targetDomain }, edges: [target] }
            }
            else
            if (type === 2) {
                const domain = data.slice(0, -1).toLowerCase()
                ...
                return { type: 'domain', label: domain, props: { domain, type: 'NS', targetDomain }, edges: [target] }
            }
            else
            if (type === 15) {
                ...
                return { type: DOMAIN_TYPE, label: domain, props: { domain, type: 'MX', targetDomain }, edges: [target] }
            }
            else
            if (type === 16) {
                ...
                return { type: 'string', label: string, props: { string, type: 'TXT', domain }, edges: [target] }
            }
            else {
                const string = data
