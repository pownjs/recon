...
const securitytrailsSuggestions = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }) {
        ...
        const { responseBody } = await scheduler.tryRequest({ uri: `https://securitytrails.com/app/api/autocomplete/domain/${search}` })
...
const securitytrailsDomainReport = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }) {
        ...
        const { responseBody } = await scheduler.tryRequest({ method: 'POST', uri: `https://securitytrails.com/app/api/domain/info/${search}` })
