...
const pwndbSearch = class extends Transform {
    ...
    async handle({ id: source = '', label = '', type = '' }) {
        ...
        const { responseBody } = await this.scheduler.tryRequest(5, { method: 'POST', uri: 'http://pwndb2am4tzkvold.onion.pet/', body })
        ...
        if (preMatch) {
            ...
            while ((arrayMatch = reg.exec(preMatch[1])) !== null) {
                ...
                if (user === 'donate' && domain === 'btc.thx' && password === '12cC7BdkBbru6JGsWvTx4PPM5LjLX8g49X') {
                    continue
                }
                ...
                const nodeId = this.makeId(PWNDB_ACCOUNT_TYPE, label)
