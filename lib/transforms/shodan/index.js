...
const { isDomain, isIpv4, isIpv6, isSubdomainOf } = require('../../detect')
const { BRAND_TYPE, ORG_TYPE, DOMAIN_TYPE, SUBDOMAIN_TYPE, SANDOMAIN_TYPE, IPV4_TYPE, IPV6_TYPE, PORT_TYPE } = require('../../types')
...
class ShodanTransform extends Transform {
    ...
    async handle({ id: source = '', type = '', label = '' }, { shodanKey = process.env.SHODAN_KEY, pages, ...options }) {
        if (!shodanKey) {
        ...
        for (;;) {
            ...
            matches.forEach((match) => {
                ...
                results.push({ id: portId, type: PORT_TYPE, label: portLabel, props: { port, transport, ssl: ssl ? true : false }, edges: [ipId] })

                hostnames.forEach((domain) => {
                    domain = normalizeDomain(domain)

                    const edges = [reportId, ipId]
