...
class ShodanTransformer extends Transformer {
    getQuery() {
    ...
    async handle({ id: source = '', label = '' }, { shodanKey = '' }) {
        ...
        while (true) {
            ...
            matches.forEach(({ ip_str: ipv4, port, ssl, hostnames }) => {
                ...
                results.push({ id: portId, type: PORT_TYPE, label: port, props: { port, ssl: ssl ? true : false }, edges: [ipv4Id] })
...
const shodanOrgSearch = class extends ShodanTransformer {
    static get alias() {
...
const shodanSslSearch = class extends ShodanTransformer {
    static get alias() {
