    ...
    async handle({ id: source = '', label = '' }, { shodanKey = process.env.SHODAN_KEY, ...options }) {
        ...
        for (;;) {
            ...
            matches.forEach((match) => {
                ...
                const { ip_str, port, ssl, hostnames, timestamp: seen } = match
                ...
                results.push({ id: portId, type: PORT_TYPE, label: portLabel, props: { port, ssl: ssl ? true : false, timestamp }, edges: [nodeId] })

                hostnames.forEach((domain) => {
                    domain = normalizeDomain(domain)

                    results.push({ type: DOMAIN_TYPE, label: domain, props: { domain, timestamp }, edges: [nodeId, { source, type: SUBDOMAIN_TYPE }] })
                })
            })
