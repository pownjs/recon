...
const pksLookupKeys = class extends Transformer {
    ...
    async handle({ id: target = '', label = '' }, options) {
        ...
        while (match) {
            ...
            if (key && uri) {
                const keyNode = { type: 'pks:key', label: key, props: { uri, key }, edges: [target] }
                ...
                addresses.forEach((address) => {
                    ...
                    const emailNode = { type: 'email', label: email, props: { email, name }, edges: [keyNode.id] }

                    results.push(emailNode)

                    if (name) {
                        results.push({ type: 'person', label: name, props: { name, email }, edges: [emailNode.id] })
                    }
                })
            }
