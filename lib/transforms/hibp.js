...
const hibpReport = class extends Transform {
    ...
    async run(items, options) {
        ...
        const results = await Promise.all(items.map(async({ id: target = '', label = '' }) => {
            ...
            return breaches.map(({ Name: name, Description: description, LogoPath: logoPath }) => {
                return { id: this.makeId('hvibp:report', name), type: 'hvibp:report', label: name, icon: logoPath, props: { name, description }, edges: [target] }
            })
        }))
