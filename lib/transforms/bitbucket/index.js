...
const bitbucketListRepos = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }) {
        ...
        const { responseBody } = await bitbucketApiScheduler.tryRequest({ uri: `https://api.bitbucket.org/2.0/repositories/${search}` })
...
const bitbucketListSnippets = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }) {
        ...
        const { responseBody } = await bitbucketApiScheduler.tryRequest({ uri: `https://api.bitbucket.org/2.0/snippets/${search}` })
...
const bitbucketListTeamRepos = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await bitbucketApiScheduler.tryRequest({ uri: `https://api.bitbucket.org/2.0/teams/${search}/repositories` })
...
const bitbucketListTeamMembers = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await bitbucketApiScheduler.tryRequest({ uri: `https://api.bitbucket.org/2.0/teams/${search}/members` })
