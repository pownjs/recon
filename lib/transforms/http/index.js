...
const httpFingerprint = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, { timeout = DEFAULT_TIMEOUT }) {
        ...
        const { responseCode, responseHeaders, responseBody } = await scheduler.tryRequest({ uri: label, rejectUnauthorized: false })
    ...
    async run(item, { timeout = DEFAULT_TIMEOUT, concurrency = DEFAULT_CONCURRENCY }) {
        return await super.run(item, { timeout }, concurrency)
    }
}
