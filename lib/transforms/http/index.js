...
const httpFingerprint = class extends Transform {
    ...
    async handle({ id: source = '', label = '', props, ...rest }, { timeout = DEFAULT_TIMEOUT, augment = DEFAULT_AUGMENT, defuse = DEFAULT_DEFUSE, follow = DEFAULT_FOLLOW, responseBodySniffSize = DEFAULT_RESPONSE_BODY_SNIFF_SIZE }) {
        const results = []
        ...
        const { responseCode, responseHeaders, responseBody } = await this.scheduler.request({ uri, headers, timeout, downloadLimit: DEFAULT_RESPONSE_BODY_SNIFF_SIZE, follow, rejectUnauthorized: false })
        ...
        if (augment) {
            results.push({ id: source, label, props: { ...props, responseCode, responseHeaders, responseBodySniff, title, software, sha1 }, ...rest })
        }
