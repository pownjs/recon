...
const httpFingerprint = class extends Transform {
    ...
    async handle({ id: source = '', label = '', props, ...rest }, { timeout = DEFAULT_TIMEOUT, augment = DEFAULT_AUGMENT, responseBodySniffSize = DEFAULT_RESPONSE_BODY_SNIFF_SIZE }) {
        const results = []
        ...
        const { responseCode, responseHeaders, responseBody } = await this.scheduler.request({ uri, headers, timeout, downloadLimit: DEFAULT_RESPONSE_BODY_SNIFF_SIZE, rejectUnauthorized: false })
        ...
        const sha1 = crypto.createHash('sha1').update(responseBodySniff).digest('hex')

        results.push({ type: SHA1_TYPE, label: sha1, props: { sha1 }, edges: [{ source, type: FINGERPRINT_TYPE }] })

        if (augment) {
            results.push({ id: source, label, props: { ...props, responseCode, responseHeaders, responseBodySniff, title, sha1 }, ...rest })
        }
