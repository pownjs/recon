...
const httpFingerprint = class extends Transformer {
    ...
    async handle({ id: target = '', label = '' }, { timeout = DEFAULT_TIMEOUT }) {
        const results = []
        ...
        if (responseCode) {
            results.push({ type: STRING_TYPE, label: `${responseCode}/http`, props: { code: responseCode }, edges: [target] })
        }

        if (server) {
            results.push({ type: STRING_TYPE, label: `${server}`, props: { server }, edges: [target] })
        }

        if (contentType) {
            results.push({ type: STRING_TYPE, label: `${contentType}`, props: { contentType }, edges: [target] })
        }
        ...
        if (text) {
            ...
            if (match) {
                ...
                results.push({ type: STRING_TYPE, label: `${softwareVersion}`, props: { softwareVersion }, edges: [target] })
            }
        }
