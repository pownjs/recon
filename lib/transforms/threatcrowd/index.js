...
const threatcrowdDomainReport = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await scheduler.tryRequest({ uri: `https://www.threatcrowd.org/searchApi/v2/domain/report/?${query}`, headers: this.headers })
...
const threatcrowdIpReport = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await scheduler.tryRequest({ uri: `https://www.threatcrowd.org/searchApi/v2/ip/report/?${query}`, headers: this.headers })
