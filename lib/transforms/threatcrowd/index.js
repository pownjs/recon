...
const threatcrowdDomainReport = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await this.scheduler.tryRequest({ uri: `https://www.threatcrowd.org/searchApi/v2/domain/report/?${query}` })
...
const threatcrowdIpReport = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await this.scheduler.tryRequest({ uri: `https://www.threatcrowd.org/searchApi/v2/ip/report/?${query}` })
