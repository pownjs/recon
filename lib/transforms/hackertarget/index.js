...
const hackertargetOnlinePortScan = class extends Transformer {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        return lines.map((line) => {
            const [_port, _state, _service] = line.split(/\s+/)

            const port = _port.toUpperCase()
            const state = _state.toLowerCase()
            const service = _service.toLowerCase()

            return { type: PORT_TYPE, label: port, props: { port, state, service }, edges: [source] }
        })
    }
}
