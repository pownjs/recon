...
const { DOMAIN_TYPE, SUBDOMAIN_TYPE, IPV4_TYPE, IPV6_TYPE, PORT_TYPE } = require('../../types')
...
const hackertargetReverseIpLookup = class extends Transform {
    ...
    async handle({ id: source = '', label = '', type: sourceType = '' }, options) {
        ...
        const { responseBody } = await scheduler.tryRequest({ uri: `http://api.hackertarget.com/reverseiplookup/?${query}`, headers: this.headers })
...
const hackertargetOnlinePortScan = class extends Transform {
    ...
    async handle({ id: source = '', label = '' }, options) {
        ...
        const { responseBody } = await scheduler.tryRequest({ uri: `https://api.hackertarget.com/nmap/?${query}`, headers: this.headers })
