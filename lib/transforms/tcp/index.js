...
const tcpPortScan = class extends Transform {
    ...
    async check(port, host, timeout = DEFAULT_TIMEOUT, banners = DEFAULT_BANNERS) {
    ...
    async handle([{ id: source = '', label = '' }, port], { timeout = DEFAULT_TIMEOUT, banners = DEFAULT_BANNERS }) {
        const results = []

        const [open, banner] = await this.check(port, label, timeout, banners)
