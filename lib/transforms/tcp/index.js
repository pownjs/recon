...
const tcpPortScan = class extends Transform {
    ...
    async handle([{ id: source = '', label = '' }, port], options) {
        ...
        if (open) {
            ...
            results.push({ id: portId, type: PORT_TYPE, label: portLabel, props: { port, protocol, services }, edges: [source] })
            ...
            if (certificate) {
                ...
                results.push({ id: certificateId, type: CERTIFICATE_TYPE, label: certificateLabel, props: { certificate, port }, edges: [source, portId] })
            }
        }
