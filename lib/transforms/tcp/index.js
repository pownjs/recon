...
const { IPV4_TYPE, IPV6_TYPE, PORT_TYPE, BANNER_TYPE, CERTIFICATE_TYPE } = require('../../types')
...
const tcpPortScan = class extends Transform {
    ...
    async handle([{ id: source = '', label = '' }, port], options) {
        ...
        if (open) {
            ...
            results.push({ id: portId, type: PORT_TYPE, label: portLabel, props: { ...props, port, protocol, services }, edges: [source] })

            if (banner) {
                const bannerLabel = crypto.createHash('sha1').update(banner).digest('hex')
                const bannerId = this.makeId(BANNER_TYPE, bannerLabel)

                results.push({ id: bannerId, type: BANNER_TYPE, label: bannerLabel, props: { banner }, edges: [source, portId] })
            }
