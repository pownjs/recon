...
const DEFAULT_EXTRACT_CERTIFICATE = false
...
const tcpPortScan = class extends Transform {
    ...
    static get options() {
        return {
            ...
            extractCertificate: {
                description: 'Extract certificate as a separate node',
                type: 'boolean',
                default: DEFAULT_EXTRACT_CERTIFICATE
            }
        }
    }
    ...
    async handle([{ id: source = '', label = '' }, port], { extractCertificate = DEFAULT_EXTRACT_CERTIFICATE, ...options }) {
        const results = []

        const [open, banner, props, certificate] = await this.check({ port: parseInt(port), host: label, ...options })
        ...
        if (open) {
            ...
            const portProps = { ...props, port, protocol, services, banner }

            if (certificate && !extractCertificate) {
                props.certificate = certificate
            }

            results.push({ id: portId, type: PORT_TYPE, label: portLabel, props: portProps, edges: [source] })
            ...
            if (certificate && extractCertificate) {
                ...
                results.push({ id: certificateId, type: CERTIFICATE_TYPE, label: certificateLabel, props: { certificate }, edges: [source, portId] })
            }
        }
